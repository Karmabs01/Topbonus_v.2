...
window.onload = function () {
  ...
  var gameConfig = {
    ...
    transparent: true, // ⬅️ Делаем фон полностью прозрачным
      ...
      this.tweens.add({
        ...
        onComplete: function (tween) {
          ...
          this.tweens.add({
            ...
            callbackScope: this,
            onComplete: function (tween) {
              // По окончании основного спина запускаем дополнительный tween для обратного поворота
              this.showCoins();
              this.tweens.add({
                targets: [this.wCont],
                angle: this.wCont.angle - backDegrees,
                ...
                onComplete: function (tween_1) {
                  // Здесь заканчивается вся анимация, и можно обрабатывать результат
                  const winResult = wheelConfig.sectors[rand_sector].win;
                  this.prizeText.setText(winResult); // Отображаем текст результата
            
                  // Отправляем сообщение родительскому окну о завершении спина с результатом выигрыша
                  parent.postMessage(
                    { event: "spinComplete", prize: winResult },
                    "*"
                  );

                  console.log("spin complete2");
                  this.canSpin = true; // разрешаем новый спин
                  this.animLightSector(); // запускаем анимацию подсветки
                  this.win_clip.play(); // проигрываем звук победы
                },
              });
            },
          });
